        ...
        <div class="itemCard colorGradientBackground">
            ...
            <div>
                <p :class="{ showing: showDescription, hidden: !showDescription }">
                    {{ description }}
                    <button class="readmoreBtn" @click="openWikiArticle()">Read more...</button>
                </p>
...
export default {
    ...
    methods: {
        ...
        imageFetch: async function () {
            const response = await fetch("https://de.wikipedia.org/w/api.php?action=query&formatversion=2&format=json&prop=pageimages%7Cpageterms&pithumbsize=1000&titles=" + this.title + "&origin=*")

            const json = await response.json()

            //this.$el.querySelector("img").src = json.query?.pages[0]?.thumbnail?.source
            this.$el.querySelector('#itemThumbnail').style.backgroundImage = `URL(${json.query?.pages[0]?.thumbnail?.source})`
        },
